...
(function() {
  ...
  exports.setRoot = function(root, callback) {
    root.parentId = null;
    root.lft = 1;
    root.rgt = 2;
    return wongo.save(root, callback);
  };

  exports.addNode = function(node, parentId, callback) {
    return wongo.findById(node._type, parentId, function(err, parent) {
      node.parentId = parentId;
      node.lft = parent.rgt;
      node.rgt = node.lft + 1;
      return async.parallel([
        function(done) {
          return wongo.save(node, done);
        }, function(done) {
          var values, where;
          where = {
            lft: {
          ...
          return wongo.update(node._type, where, values, done);
        }, function(done) {
          var values, where;
          where = {
            lft: {
